...
function parse(template, params, tpl_path){
	var parser = new Parser(template, params, tpl_path);
	return parser.tokenize();
}
nsmarty.Parser = Parser.nSmarty;

Object.defineProperty(nsmarty, 'tpl_path', {
	get         : function (){
		throw new Error('please set nSmarty.tpl_path first.');
	},
	enumerable  : true,
	configurable: true
})
...
nsmarty.fs = function (filename, callback){
	filename = filename.indexOf('/') === 0 ||
	           filename.indexOf(':\\') === 1? filename : path.join(nsmarty.tpl_path, filename);
...
nsmarty.compile = function (filename, params, callback, unique){
	...
	nsmarty.fs(filename, function (err, contents){
		...
		parsed = parse(contents, params, nsmarty.tpl_path);
		// console.log(parsed);
...
nsmarty.renderText = function (template, params, partials){
	...
	for(name in partials){
		if(partials.hasOwnProperty(name) && !partials[name].tokens){
			partials[name] = parse(partials[name]);
		}
	}
...
nsmarty.clearCache = function (templateName){
	if(templateName){
		...
		delete nsmarty.Parser.cache_files[templateName];
	} else{
		nsmarty.cache = {};
